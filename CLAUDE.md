...
#### Branch-mode overlap collapse (`o`)

When a file is changed both in a commit and again in the working tree, plain stacking (`composeBranch` → `mergeFileDiffs`) appends a `[branch]` hunk and a `[staged]`/`[unstaged]` hunk for the same lines, exposing an intermediate state (the committed value shown as the working-tree hunk's base) that never exists on disk. Overlap collapse replaces those with a single net `[overlap]` hunk (`SourceOverlap`), diffed straight from the committed range's base ref to the working tree.

- `branchDiff(contextLines, hideWhitespace, depth, overlap bool)` takes a runtime flag. `overlap == false` → `composeBranch` (stack, unchanged); `true` → `composeOverlap` (in `compose_strategy.go`). The remote-divergence case (`mergeBase == HEAD`) always stacks — there's no local commit layer to overlap. `BranchDiffDepth` gained the `overlap` param; `BranchDiff`/`BranchDiffOptions` stay stack-only (`false`) wrappers used by the initial paint's fallback and the `diff` subcommand.
- **Per-file, not whole-diff.** `composeOverlap` collapses only files whose committed and working-tree edits touch **overlapping line ranges**; files that changed at disjoint lines keep their tagged stack (so provenance is preserved where it isn't misleading). Overlap is decided by interval intersection in **HEAD coordinates** — the committed range's new-side ranges (`newSideRanges`) vs `git diff HEAD`'s old-side ranges (`oldSideRanges`). Collapsed hunks come from `git diff <from>` (`from` = merge-base or `HEAD~depth`), re-tagged `SourceOverlap`.
- **Cost is conditional.** Only files present in *both* the committed diff and the working-tree diff can overlap; if that candidate set is empty, `composeOverlap` returns the plain stack with **zero** extra git calls. When non-empty, the two helper diffs (`git diff HEAD`, `git diff <from>`) are scoped to just the candidate paths (`-- <paths>`). This matters given many concurrent instances on shared worktrees.
- Model state: `mergeOverlap bool` (default **true**), toggled by `o` (git-only key, no-op in file review), threaded into the `ModeBranch` branch of `loadDiffWithOptions`. `main.go`'s initial branch paint passes `overlap=true` to match. The toggle sets a transient `statusMsg` ("Overlap collapse: on/off").
- **Rendering caveat:** collapsed hunks carry `SourceOverlap`, not `SourceBranch`/`SourceStaged`/`SourceUnstaged`, so per-hunk stage/unstage/discard on an `[overlap]` hunk isn't attributable to a single layer. The `[overlap]` tag matches the other source tags' italic styling but renders in its own accent color (`overlapFg`, a purple, via `hunkOverlapTagStyle`) to distinguish it from the dim source tags.
